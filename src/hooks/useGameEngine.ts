...
export function useGameEngine() {
  ...
  const startGame = useCallback(() => {
      ...
      setState((prev) => {
        ...
        const accuracy =
          prev.totalChars > 0
            ? Math.round((prev.correctChars / prev.totalChars) * 100)
            : 100;
        return { ...prev, wpm, accuracy, elapsed };
      });
    }, 500);
    ...
    healthTimerRef.current = setInterval(() => {
      ...
      const drainAmount = HEALTH_DRAIN_PER_TICK + (cur.level - 1) * 0.5;
      const newHealth = Math.max(0, cur.health - drainAmount);
      setState((prev) => ({ ...prev, health: newHealth }));
      if (newHealth <= 0) {
        endGame();
      }
    }, 1500);
...
const handleInput = useCallback((input: string) => {
  setState(prev => {
    ...
    if (isCorrect) {
      ...
      if (combo > 0 && combo % 50 === 0) onComboMilestone.current?.(combo);
